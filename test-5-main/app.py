...
# --- CẤU HÌNH NHẬN THÔNG BÁO TELEGRAM (Chỉ thay đổi chỗ này) ---
st.sidebar.markdown("---")
...
if st.session_state.is_running:
    ...
    # BẮN CẢNH BÁO TELEGRAM KHI VƯỢT NGƯỠNG (Chỉ thay đổi hàm gọi tại đây)
    if status != "🟩 Lý tưởng":
...
if not df_display.empty:
    # Bản sao dữ liệu phục vụ biểu đồ tuyến tính dòng thời gian (Xuôi dòng thời gian)
    ...
    # Khối gọi hàm phân tích theo buổi nguyên bản từ analytics.py của bạn
    df_f_blk = df_display.copy()
    report_list = analyze_day_by_blocks_rt(df_f_blk, vpd_min, vpd_max)
...
# Tự động và liên tục tải lại ứng dụng sau mỗi 2.5 giây khi đang kích hoạt real-time
if st.session_state.is_running:
    import time
    time.sleep(2.5)
    st.rerun()
